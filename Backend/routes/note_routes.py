...
def get_note_by_link(random_link):
    ...
    cursor = conn.cursor()
    full_link = f"http://127.0.0.1:5000/{random_link}"
...
def save_note():
    ...
    note_link = f"http://127.0.0.1:5000/{random_link}"
    date = datetime.now()
...
def create_or_get_note():
    ...
    note_link = data.get('note_link')  # Check if a link already exists
    ...
    try:
        ...
        return jsonify({'message': 'Note created', 'note_link': f"http://localhost:5173/{note_link}"}), 201
...
def get_note(note_link):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT notes FROM notes WHERE note_link = %s", (note_link,))
        result = cur.fetchone()
        conn.close()
